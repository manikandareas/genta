...
```bash
git clone https://github.com/manikandareas/genta.git
cd genta
```
...
The backend uses environment variables prefixed with `GENTA_`. Key variables include:

- `GENTA_DATABASE_*` - PostgreSQL connection settings
- `GENTA_SERVER_*` - Server configuration
- `GENTA_AUTH_*` - Authentication settings
- `GENTA_REDIS_*` - Redis connection
- `GENTA_EMAIL_*` - Email service configuration
- `GENTA_OBSERVABILITY_*` - Monitoring settings
